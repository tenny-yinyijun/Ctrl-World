...
def load_videos_with_clip_samples(dataset_path, model, processor, num_samples=3,
                                  horizon_length=4, frame_spacing=2, device='cuda',
                                  random_seed=42):
    """
    Load all videos from the dataset and extract CLIP features from short-horizon samples.
    ...
    Returns:
        samples: list of dicts, each containing:
            ...
            - 'view_id': view ID (0, 1, or 2)
            - 'sample_id': sample index within this trajectory/view
...
def prepare_sample_features(samples_dict, view_ids=[0, 1, 2]):
    """
    Prepare features for t-SNE by combining views within each sample.

    Args:
        samples_dict: dict mapping dataset_name -> list of sample dicts
        view_ids: which views to include when computing sample features

    Returns:
        features: np.array of shape [N, D] where N is number of samples
        labels: list of (dataset_name, traj_id, sample_id, view_str) tuples
    """
    features_list = []
    labels = []

    # Organize samples by (dataset, traj_id, sample_id)
    organized = {}
    for dataset_name, samples in samples_dict.items():
        for sample in samples:
            if sample['view_id'] not in view_ids:
                continue

            key = (dataset_name, sample['traj_id'], sample['sample_id'])
            if key not in organized:
                organized[key] = {}
            organized[key][sample['view_id']] = sample['features']

    # Process each unique sample
    for (dataset_name, traj_id, sample_id), views in sorted(organized.items()):
        # Check if all required views are present
        if not all(vid in views for vid in view_ids):
            continue

        # Concatenate the views to create a single feature vector
        view_features = []
        ...
        sample_feature = np.concatenate(view_features)
        features_list.append(sample_feature)

        # Create view description string
        view_str = '+'.join([str(v) for v in sorted(view_ids)])
        labels.append((dataset_name, traj_id, sample_id, view_str))
...
def plot_tsne(features, labels, title, output_path, view_description, horizon_info):
    """
    ...
    unique_datasets = sorted(set(dataset_name for dataset_name, _, _, _ in labels))
    ...
    dataset_counts = {dataset: sum(1 for d, _, _, _ in labels if d == dataset)
                     for dataset in unique_datasets}

    # Plot each sample
    for i, (dataset_name, traj_id, sample_id, view_str) in enumerate(labels):
        x, y = embedded[i]
    ...
    unique_trajs = len(set((d, t) for d, t, _, _ in labels))
...
def main():
    parser = argparse.ArgumentParser(
        description='Create t-SNE visualizations using CLIP features from short-horizon video samples'
    )
    ...
    # Load all datasets and extract CLIP features from samples
    all_samples = {}
    for dataset_name in args.dataset_names:
        ...
        # Load samples from all videos
        samples = load_videos_with_clip_samples(
        ...
        print(f"Extracted {len(samples)} total samples")
        ...
        print(f"  View 0 (Left View): {view_counts[0]} samples")
        print(f"  View 1 (Right View): {view_counts[1]} samples")
        print(f"  View 2 (Wrist View): {view_counts[2]} samples")
    ...
    print("="*70)
    try:
        features_all, labels_all = prepare_sample_features(all_samples, view_ids=[0, 1, 2])
        plot_tsne(
            features_all,
            labels_all,
            't-SNE: Short-Horizon Sample Comparison (CLIP Features)',
            output_dir / f'{output_prefix}_tsne_clip_shorthor_all_views{suffix}.png',
            'CLIP features from Left + Right + Wrist views',
            horizon_info
        )
    except ValueError as e:
        print(f"Skipping all views visualization: {e}")
    ...
    print("="*70)
    try:
        features_01, labels_01 = prepare_sample_features(all_samples, view_ids=[0, 1])
        plot_tsne(
            features_01,
            labels_01,
            't-SNE: Short-Horizon Sample Comparison (CLIP Features)',
            output_dir / f'{output_prefix}_tsne_clip_shorthor_lr_views{suffix}.png',
            'CLIP features from Left + Right views only',
            horizon_info
        )
    except ValueError as e:
        print(f"Skipping L+R views visualization: {e}")

    print("\n" + "="*70)
    print("Done! Visualizations saved to:")
    print(f"  - {output_dir / f'{output_prefix}_tsne_clip_shorthor_all_views{suffix}.png'}")
    print(f"  - {output_dir / f'{output_prefix}_tsne_clip_shorthor_lr_views{suffix}.png'}")
    print("="*70)
